...
class PacienteUpdateRequest extends PacienteCreateRequest
{
    public function rules()
    {
        return array_merge(parent::rules(), [
            ...
            "PACIENTE_CPF" => [
                "required",
                "cpf",
                Rule::unique("PACIENTE", "PACIENTE_CPF")->ignore($this->PACIENTE_ID, "PACIENTE_ID")
            ],
        ]);
    }
    ...
    public function withValidator($validator)
    {
        $validator->after(function ($validator) {
            ...
            if ($cpfAtual && $cpfAtual !== $cpfInformado)
                $validator->errors()->add("PACIENTE_CPF", "O CPF do paciente não pode ser alterado ou removido.");
        });
    }
}
